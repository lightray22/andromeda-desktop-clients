...
TEST_CASE("DeriveKey", "[Crypto]")
{
    ...
    REQUIRE(Crypto::DeriveKey(password, salt, 16) == 
        std::string("\x6b\xf2\xe7\xa9\x9d\x16\xa8\x18\x42\xbf\x69\x4f\xc6\xaa\xe0\x64",16));

    REQUIRE(Crypto::DeriveKey(password, salt, 24) == 
        std::string("\x0d\x34\x72\x91\xce\x9e\xca\x4b\x88\xe9\xbe\x36\xaf\x8a\x05\xae\xac\x62\x4c\x72\x4b\xd1\x7f\x5d",24));

    REQUIRE(Crypto::DeriveKey(password, salt, 32) == 
        std::string("\xa4\x9d\xd9\x7a\x61\x7a\xcd\xc0\x3b\xbd\x4f\x30\x03\xb9\xd5\xd4\x94\xc7\xff\x69\xbd\x22\x21\x84\x95\xe6\xdd\xe7\x29\xf6\xf1\x1f",32));
}
...
TEST_CASE("CryptoSecret", "[Crypto]")
{
    ...
    const std::string msg { "my super secret data..." };
    ...
    const std::string badkey { "1113456789ABCDEF0123456789ABCDEF" }; // 32 bytes
...
TEST_CASE("CryptoPublic", "[Crypto]")
{
    ...
    const std::string priv1("\x29\x20\x53\x10\x9b\x4f\x6f\x89\xeb\xbd\xe2\x77\x71\xdc\x4b\x40\xbd\xdd\xff\xb7\xc4\x20\x06\x4c\xd8\x6d\x1e\x80\x5f\x26\x59\xa4",32);
    ...
    const std::string priv2("\xcf\x05\xa7\x0e\x08\x8d\xb9\x2b\xf3\xa0\xee\x12\xe2\xe8\x5f\x9f\x6f\x73\xbe\xa2\xd7\x3a\xb3\xc3\xd9\x0c\x78\x91\x23\xf3\xd3\xe0",32);
