...
TEST_CASE("Compares", "[QueryBuilder]")
{
...

    {
        ...
        REQUIRE(q.GetParams() == MixedParams{{":d0",5}});
...

    {
        ...
        REQUIRE(q.GetParams() == MixedParams{{":d0",0}}); 
        REQUIRE(q.GetText() == "WHERE mykey > :d0");
    }

    {
        QueryBuilder q; q.Where(q.Equals("mykey","myval"));
        REQUIRE(q.GetParams() == MixedParams{{":d0","myval"}}); 
        REQUIRE(q.GetText() == "WHERE mykey = :d0");
    }
...
TEST_CASE("Combos", "[QueryBuilder]")
{
...

    {
        ...
        REQUIRE(q.GetParams() == MixedParams{{":d0","myval1"},{":d1","myval2"}});
...
TEST_CASE("AutoWhereAnd", "[QueryBuilder]")
{
    ...
    REQUIRE(q.GetParams() == MixedParams{{":d0",3},{":d1",4}}); 
